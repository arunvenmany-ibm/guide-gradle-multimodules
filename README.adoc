...
The [hotspot= test file=1]`test` task configuration is already included in the [hotspot file=1]`ear/build.gradle` file for you. It passes the [hotspot=testHttpPort file=1]`http.port` value from the running Liberty server to the tests so they can connect to the application during execution.
